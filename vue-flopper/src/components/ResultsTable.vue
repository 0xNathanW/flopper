...
<script setup lang="ts">
    ...
    const maxEv = computed(() => {
        ...
        if (props.tableMode !== "chance") {
            ...
            if (!results || results.empty) return 0;
            return Math.max(...results.ev[playerIndex].map((v) => Math.abs(v)));
        } else {
            const reports = props.chanceReport;
    ...
    const numActions = computed(() => {
        let data: Results | ChanceReport | null;
        if (props.tableMode !== "chance") {
            data = props.results;
        } else {
            data = props.chanceReport;
        }

        const spot = props.selectedNode;
        return (
            (spot.type === "player" &&
    ...
    const columns = computed(() => {
        ...
        if (props.tableMode !== "chance") {
            ...
            ret.push({ label: props.chanceType, type: "card" });
            ret.push({
    ...
    const resultsFiltered = computed(() => {
        ...
        if (props.tableMode !== "chance") {
            dataRow = (index: number) => {
                ...
                const normalizer = results.normaliser[playerIndex][index];
                ...
                ret.push(results.normaliser[playerIndex][index]);
        ...
        if (props.hoverContent) {
            for (const index of props.hoverContent.idxs) {
    ...
    const actionColors = computed(() => {
        ...
        const spot = props.selectedNode as PlayerNode;
        return spot.actions.map((a) => a.colour).reverse();
    });
    ...
    const exportSummary = async () => {
        ...
        if (props.tableMode !== "chance") {
            ...
            if (!props.results.empty) {
                ary.push("Equity", "EV", "EQR");
            }

            if (numActions.value > 0) {
                const actions = (props.selectedNode as PlayerNode).actions;
            ...
            const startIndex = props.results.empty
                ? INDEX_STRATEGY_BASE
                : INDEX_EQUITY;
                ...
                <tbody>
                    ...
                    <tr v-if="resultsRendered.length === 0">
                        ...
                        >
                            {{
                                tableMode === "chance"
                                    ? `${chanceType} reports not available`
                                    : "No results"
                            }}
                        </td>
                    </tr>
