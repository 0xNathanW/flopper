...
<script setup lang="ts">
    ...
    import { average, toFixed1, toFixed, toFixedAdaptive } from "../util";
    import { Results, ActionNode, ChanceNode } from "../typing";
    ...
    const player = computed(() => {
        ...
        return props.selectedNode.player;
    });
    ...
    const combos = computed(() => {
        ...
        for (let i = 0; i < 2; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                const normalizer = results.normaliser[i][j];
    ...
    const equity = computed(() => {
        const results = props.results;
        if (results.empty) return [Number.NaN, Number.NaN];
        return [
            average(results.equity[0], results.normaliser[0]),
            average(results.equity[1], results.normaliser[1]),
        ];
    });
    ...
    const ev = computed(() => {
        ...
        if (results.empty) return [Number.NaN, Number.NaN];
        return [
            average(results.ev[0], results.normaliser[0]),
            average(results.ev[1], results.normaliser[1]),
        ];
    });
