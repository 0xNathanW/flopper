<script setup lang="ts">
    ...
    import { cardText, cardPairOrder, toFixed1, toFixedAdaptive } from "../util";
    ...
    const playerIndex = computed(() => {
        if (props.selectedNode.type === "player" && !props.selectedChance) {
            return props.selectedNode.player === "oop" ? 0 : 1;
        } else {
            return -1;
        }
    });
    ...
    const tupleData = computed(() => {
        if (props.results.empty) return null;

        const results = props.results;
        const content = props.displayOpts.contentGraphs;
        ...
        for (let player = 0; player < 2; ++player) {
            ...
            const normalizer = results.normaliser[player];
    ...
    const strategyItems = computed(() => {
        ...
        const actionColors = (props.selectedNode as PlayerNode).actions.map(
            (a) => a.colour
        );
...
<template>
    <div class="flex w-full h-full">
        <div v-if="!results.empty" class="flex flex-col h-full" style="flex: 4">
        ...
        <ResultsTable
            style="flex: 3"
            table-mode="graphs"
            :graph-type="displayOpts.contentGraphs"
            :cards="cards"
