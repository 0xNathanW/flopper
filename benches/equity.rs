...
fn generate_random_board(num_cards: usize) -> Board {
    if num_cards == 0 {
...
fn benchmark_monte_carlo(c: &mut Criterion) {
    ...
    group.bench_function("preflop_2p", |b| {
        b.iter_with_setup(
            || {
                ...
                let board = generate_random_board(0);
                EquityParams {
                    ranges,
                    board,
                    lookup: &lookup_table,
    ...
    group.bench_function("flop_2p", |b| {
        b.iter_with_setup(
            || {
                let ranges = fixed_ranges(2);
                let board = generate_random_board(3);
                EquityParams {
                    ranges,
                    board,
                    lookup: &lookup_table,
    ...
    group.bench_function("turn_2p", |b| {
        b.iter_with_setup(
            || {
                let ranges = fixed_ranges(2);
                let board = generate_random_board(4);
                EquityParams {
                    ranges,
                    board,
                    lookup: &lookup_table,
    ...
    group.bench_function("river_2p", |b| {
        b.iter_with_setup(
            || {
                let ranges = fixed_ranges(2);
                let board = generate_random_board(5);
                EquityParams {
                    ranges,
                    board,
                    lookup: &lookup_table,
